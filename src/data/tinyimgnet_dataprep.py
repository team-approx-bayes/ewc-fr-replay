"""
File copied from: https://github.com/Mattdl/CLsurvey/blob/master/src/data/tinyimgnet_dataprep.py
...
def create_training_classes_file(root_path):
    ...
    with open(os.path.join(root_path, 'classes.txt'), 'w') as classes_file:
        for class_dir in get_immediate_subdirectories(os.path.join(root_path, 'train')):
...
def divide_into_tasks(root_path, task_count=10):
    ...
    for subset in subsets:
        ...
        for initial_class in (range(0, len(lines), nb_classes_task)):
            ...
            classes.sort()
            class_to_idx = {classes[i]: initial_class + i for i in range(len(classes))}
