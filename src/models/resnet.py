...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.bn3 = norm_layer(
            planes * self.expansion, momentum=norm_stat_momentum
        )
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
...
class ResNet(nn.Module):
    def __init__(
        ...
        self.layer1 = self._make_layer(
            ...
            norm_stat_momentum=norm_stat_momentum
        )
    ...
    def _make_layer(
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                norm_layer(
                    planes * block.expansion, momentum=norm_stat_momentum
                ),
            )
...
def ResNext50_32x4d(**kwargs):
    model = ResNet(
        Bottleneck, [3, 4, 6, 3], groups=32, width_per_group=4, **kwargs
    )
    return model
...
def ResNext101_32x8d(**kwargs):
    model = ResNet(
        Bottleneck, [3, 4, 23, 3], groups=32, width_per_group=8, **kwargs
    )
    return model
